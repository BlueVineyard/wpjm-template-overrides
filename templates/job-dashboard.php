...
<style>
    #job-manager-job-dashboard .jm-ui-button {
        background: #ff8200;
        border-color: #ff8200;
    }
    ...
    #job-manager-job-dashboard .jm-ui-button:hover span {
        color: #ff8200;
    }
    ...
    #job-manager-job-dashboard .jm-dashboard-job-column-label {
        font-weight: 700;
    }

    #job-manager-job-dashboard .job-title {
        color: #101010;
    ...
    <div class="jm-dashboard__intro">
        ...
        <div class="jm-dashboard__actions">
            <?php if (job_manager_user_can_submit_job_listing()) : ?>
                <a class="jm-ui-button" href="/dashboard/add-listing/"><span><?php esc_html_e('Add Job', 'wp-job-manager'); ?></span></a>
            <?php endif; ?>
        </div>
    </div>
                    ...
                    <div class="jm-dashboard-job">
                        ...
                        <div class="jm-dashboard-job-column actions job-dashboard-job-actions">
                            ...
                            $edit_url = add_query_arg([
                                'action' => 'edit',
                                'job_id' => $job->ID,
                            ], site_url('add-listing'));
                            if ('draft' == $job->post_status) {
                            ?>

                                <a class="jm-ui-button--outline job-dashboard-action-continue jm-dashboard-action jm-dashboard-action--primary jm-ui-button--small" href="<?php echo esc_url($edit_url) ?>"><span>Continue Submission</span></a>

                            <?php } else {
                                do_action('job_manager_job_dashboard_column_actions', $job, $job_actions[$job->ID] ?? []);
                            }
                            ?>
                            ...
                            if (! empty($job_actions[$job->ID])) {
                                ...
                                $actions_html .= '<a class="jm-dashboard-action jm-ui-button--link job-dashboard-action-edit" href="' . esc_url($edit_url) . '">Edit</a>';
                                // } 
